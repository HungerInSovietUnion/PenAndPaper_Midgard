import random

leben = random.randint(4, 8)
geschwindigkeit = random.randint(7, 14)
schwimm_geschwindigkeit = random.randint(2, 5)
staerke = random.randint(6, 9)

print()
print("Gegner wird erstellt...")
print()
print("Wolf Stats werden generiert..")
print()
print("--Leben:", leben)                                    #Leben
print()
print("--Geschwindigkeit:", geschwindigkeit)                #Geschwindigkeit
print()
print("--Schwimmgeschwindigkeit:", schwimm_geschwindigkeit) #Schwimmgeschwindigkeit
print()
print("--Stärke:", staerke)                                 #Stärke
print()

input("Beliebige Taste zum Beenden drücken...")




